...
class Molecule:
    ...
    def homogeneous(self):
        ''' returns homogeneous form of coordinates (column major form)
        for translation and rotation matrix multiplication '''
        return np.concatenate((
            self.get_coords(),
            np.ones((self.get_coords().shape[0], 1))
        ), axis=1).T
    ...
    def rotate(self, axis_seq, angles, degrees):
        ...
        R = np.concatenate((
            np.concatenate((R, np.array([0, 0, 0]).reshape(3, -1)), axis=1),
            np.array([0, 0, 0, 1]).reshape(-1, 4)
        ), axis=0)

        self.set_coords((
            Molecule.T(x, y, z)@(R@(Molecule.T(-x, -y, -z)@self.homogeneous()))
        ).T[:, 0:3])
    ...
    def crop(self, center_coord, x, y, z):
        ...
        def crop_condition(atom):
            return check_range(atom.coord[0], min_x, max_x) \
    ...
    def summary(self):
        ...
        print(f"Max distance between atoms\
        along\nx:{self.atom_range(0)}\ny:{self.atom_range(1)}\nz:{self.atom_range(2)}")
